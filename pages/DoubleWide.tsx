...
const DoubleWide: React.FC = () => {
  ...
  // Use unified filtering with default type='Double Wide'
  const { filteredHomes, filters, setFilter, clearFilters, activeFilterCount } = useHomeFilters(
    UNIFIED_INVENTORY,
    { type: 'Double Wide' }
  );

  // Get all double-wide homes for filter options
  const doubleWideHomes = useMemo(
    () => UNIFIED_INVENTORY.filter(h => h.type === 'Double Wide'),
    []
  );

  // Extract unique filter values from double-wide homes only
  const manufacturers = useMemo(
    () => ['all', ...Array.from(new Set(doubleWideHomes.map(h => h.manufacturer)))],
    [doubleWideHomes]
  );
  const bedOptions = useMemo(() => ['all', '3', '4', '5'], []);
  ...
  const groupedHomes = useMemo(() => {
    ...
    return getManufacturersWithPlaceholders(filteredHomes, 'Double Wide');
  ...
  return (
    <>
      <SEOHead
        title={SEO_CONFIG.doubleWide.title}
        description={SEO_CONFIG.doubleWide.description}
        canonical={SEO_CONFIG.doubleWide.canonical}
        ogImage={SEO_CONFIG.doubleWide.ogImage}
      />
      ...
      <div id="homes-grid" className="container mx-auto py-12 px-4 sm:px-6 lg:px-8 scroll-animate">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-8 gap-4">
          ...
          <div className="flex items-center gap-3">
            {activeFilterCount > 0 && (
              <button
                onClick={() => clearFilters({ type: 'Double Wide' })}
